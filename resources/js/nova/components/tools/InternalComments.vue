...
export default {
  ...
  computed: {
    commentsUri: () => `${this.baseCommentUri}?page=1`,
    hasComments: () => Boolean(this.data.resources.length),
    hasNextLink: () => Boolean(this.data.next_page_url),
    hasPrevLink: () => Boolean(this.data.prev_page_url),
    hasPagination: () => this.hasNextLink || this.hasPrevLink,
    queryParams: () => `&orderBy=created_at&orderByDirection=desc&viaResource=${this.resourceName}&viaResourceId=${this.resourceId}&viaRelationship=comments&relationshipType=hasMany`
  },
